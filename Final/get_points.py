...
def crop(img, bboxes):
        ...
        for bbox in bboxes:
            ...
            signs = img[int(bbox[1])-1:int(bbox[3])+1,(int(bbox[0]))-1:int(bbox[2])+1]
...
def get_point(img, crops, file):
    ...
    upper = crops["upper"]
    special_points = []
    otsu_threshold, _ = cv2.threshold(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY),0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    # create save dir
    ...
    for id,img_crop in (enumerate(img_crops)):
        ...
        _, img_bn = cv2.threshold(gray,means[id],255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        _, contours, _= cv2.findContours(img_bn,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
        ...
        for contour in contours:
            ...
            for corner in corners:
                if canny[corner[0][1]][corner[0][0]] != 0: 
                      cv2.circle(img_crop, (corner[0][0],corner[0][1]), 1, [0,0,255],3)
                      special_points.append(corner[0])
                else:
...
def main(data_path):
    txt_path = data_path + "/all_timestamp.txt"
    try:
    ...
    for line in tqdm(lines):
        file = line.strip("\n")
        file_dir = data_path + "/dataset/" + file
        csv_path = file_dir+"/detect_road_marker.csv"
        img_path = file_dir + "/raw_image.jpg"
        ...
        try:
            ...
            bboxes, conf = nms(bboxes, conf, 0.2)
        except IOError:
        ...
        sp = get_point(img, crops, file) 

if __name__ =='__main__':
    data_path = "./ITRI_dataset/seq1"
    main(data_path)